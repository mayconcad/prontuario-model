...
import br.com.saude.prontuario.model.springsecurity.entities.QRole;
import br.com.saude.prontuario.model.springsecurity.entities.QUser;
...
public class UserRepositoryImpl extends QueryDslJpaRepository<User, Long>
	...
	private static QRole qRole = QRole.role;
	...
	public List<User> buscar(Map<String, Object> params) {
		...
		if (params.containsKey("principalRole"))
			predicate
					.and(qRole.name.eq(params.get("principalRole").toString()));
		predicate.and(user.ativo.eq(true));
		return createQuery().leftJoin(user.roles, qRole).where(predicate)
				.orderBy(user.username.asc()).listDistinct(user);
	}
}