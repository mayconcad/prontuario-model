...
import br.com.saude.prontuario.model.repository.interfaces.UnidadeRepository;
...
public class UnidadeRepositoryImpl extends QueryDslJpaRepository<Unidade, Long>
	...
	@Override
	public List<Unidade> autocompletarPorResponsavel(String valor,
			long responsavelId) {
		BooleanBuilder predicate = new BooleanBuilder();
		if (responsavelId != 0)
			predicate.and(qUnidade.responsavel.id.eq(responsavelId));
		if (valor != null && !"".equals(valor))
			predicate.and(qUnidade.nome.containsIgnoreCase(valor));
		predicate.and(qUnidade.ativo.eq(true));
		return createQuery(predicate).orderBy(qUnidade.nome.asc())
				.listDistinct(qUnidade);
	}
